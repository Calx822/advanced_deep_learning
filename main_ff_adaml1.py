...
def train(args, model, device, train_loader, optimizer, epoch,plist):
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer, epoch, plist = p_below_threshold)
        test(model, device, test_loader, losslist = test_loss)
        scheduler.step()

    if args.save_model:
        torch.save(model.state_dict(), "mnist_cnn.pt")
    df = pd.DataFrame(test_loss)
    df.to_csv('loss_{}_{}.csv'.format(args.lr, args.epochs), index = False)
    df2 = pd.DataFrame(p_below_threshold)
    df2.to_csv('parameters{}_{}.csv'.format(args.lr,args.epochs), index = False)
    df3 = pd.DataFrame(accuracy_l)
    df3.to_csv('accuracies{}_{}.csv'.format(args.lr, args.epochs), index = False)
